...
/**
 * bmi08a_spi_write_wrapper - The SPI write function pointer used by BMI088 API.
 *
 ...
 */
static s8 bmi08a_spi_write_wrapper(u8 reg_addr, const u8 *data,
...
/**
 * bmi08a_spi_read_wrapper - The SPI read function pointer used by BMI088 API.
 *
 ...
 * * negative value - Error.
 */
static s8 bmi08a_spi_read_wrapper(u8 reg_addr, u8 *data,
						u32 len, void *intf_ptr)
{
	s8 err;
 ...
 * @retval non-zero failed
 */
static int bmi08a_spi_probe(struct spi_device *client)
{
	int status;
	int err = 0;
	struct bmi08a_client_data *a_client_data = NULL;
	...
	a_client_data = kzalloc(sizeof(struct bmi08a_client_data), GFP_KERNEL);
	if (a_client_data == NULL) {
		PERR("no memory available");
		err = -ENOMEM;
		goto exit_err_clean;
	}

	a_iio_spi_dev = devm_iio_device_alloc(&client->dev, sizeof(*a_client_data));
	if (!a_iio_spi_dev)
		return -ENOMEM;

	a_client_data = iio_priv(a_iio_spi_dev);
	a_client_data->device.read_write_len = 16;
	a_client_data->device.read = bmi08a_spi_read_wrapper;
	a_client_data->device.write = bmi08a_spi_write_wrapper;
	a_client_data->device.intf = BMI08_SPI_INTF;
	a_client_data->device.variant = BMI088_VARIANT;
	a_client_data->dev = &client->dev;
	a_iio_spi_dev->dev.parent = &client->dev;
	a_iio_spi_dev->name = "bmi08a";
...
static int bmi08a_spi_remove(struct spi_device *client)
{
	int err = 0;
#else
static void bmi08a_spi_remove(struct spi_device *client)
{
#endif

// #ifndef BMI08_KERNEL_6_1
#if LINUX_VERSION_CODE <= KERNEL_VERSION(6, 1, 0)
		err = bmi08a_remove(a_iio_spi_dev);
#else
		bmi08a_remove(a_iio_spi_dev);
#endif
	bmi_spi_client = NULL;
// #ifndef BMI08_KERNEL_6_1
#if LINUX_VERSION_CODE <= KERNEL_VERSION(6, 1, 0)
 ...
 */
static const struct spi_device_id bmi08a_id[] = {
	{ SENSOR_NAME, 0 },
	{ }
};
MODULE_DEVICE_TABLE(spi, bmi08a_id);

/*!
 * @brief register bmp3 device id match
 */
static const struct of_device_id bmi08a_of_match[] = {
	// { .compatible = "bmi08x", },
	{ .compatible = "bmi08a", },
	{ }
};
MODULE_DEVICE_TABLE(of, bmi08a_of_match);

/*!
 * @brief register spi driver hooks
 */
static struct spi_driver bmi08a_spi_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name  = SENSOR_NAME,
		.of_match_table = bmi08a_of_match,
	},
	.id_table = bmi08a_id,
	.probe	  = bmi08a_spi_probe,
	.remove	  = bmi08a_spi_remove,
};
 ...
 * @retval non-zero failed
 */
static int __init bmi08a_spi_init(void)
{
	return spi_register_driver(&bmi08a_spi_driver);
}
 ...
 */
static void __exit bmi08a_spi_exit(void)
{
	spi_unregister_driver(&bmi08a_spi_driver);
}
...
/*lint -e19*/
module_init(bmi08a_spi_init);
module_exit(bmi08a_spi_exit);
/*lint +e19*/
