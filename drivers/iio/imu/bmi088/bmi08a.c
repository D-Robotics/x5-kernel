 ...
 */
int8_t bmi08a_write_feature_config(const uint16_t *reg_data, struct bmi08_dev *dev)
{

    int8_t rslt;
    int8_t index = 0;
    uint8_t reg_addr = BMI08_ACCEL_DATA_SYNC_ADR;
    uint8_t len = BMI08_ACCEL_DATA_SYNC_LEN;
    uint16_t read_length = (reg_addr * 2) + (len * 2);
    uint8_t feature_data[(BMI08_ACCEL_DATA_SYNC_ADR * 2) + (BMI08_ACCEL_DATA_SYNC_LEN * 2)];
    ...
    if (rslt == BMI08_OK)
    {
        ...
        if (rslt == BMI08_OK)
        {
            ...
            if (rslt == BMI08_OK)
            {
                ...
                rslt = bmi08a_write_feature_config(&data[0], dev);
            }
        }
    }
