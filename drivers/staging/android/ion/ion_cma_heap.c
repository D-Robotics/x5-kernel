...
static struct ion_heap *__ion_cma_heap_create(struct cma *cma)
{
	 ...
	 */
	cma_heap->cma = cma;
	if (!strcmp(cma_get_name(cma), ION_CMA_NAME)) {
		cma_heap->heap.type = ION_HEAP_TYPE_DMA;
	} else if (!strcmp(cma_get_name(cma), ION_CMA_EXTRA_NAME)) {
...
static int __ion_add_cma_heaps(struct cma *cma, void *data)
{
	...
	if (strcmp(name, ION_CMA_NAME) && strcmp(name, ION_CMA_EXTRA_NAME))
		return 0;
