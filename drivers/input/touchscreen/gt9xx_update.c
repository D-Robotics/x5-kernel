...
u16 show_len;
u16 total_len;
EXPORT_SYMBOL(show_len);
EXPORT_SYMBOL(total_len);
...
static u8 gup_enter_update_judge(struct i2c_client *client, struct st_fw_head *fw_head)
{
	...
	if (update_msg.fw_total_len != fw_len) {
		...
		dev_err(&client->dev, " Default size: %d(%dK), actual size: %d(%dK)", fw_len,
			fw_len / 1024, update_msg.fw_total_len, update_msg.fw_total_len / 1024);
		return FAIL;
	}
...
static int gup_update_config(struct i2c_client *client)
{
	...
	file_cfg_len =
		gtp_ascii_to_array(fw_cfg->data, fw_cfg->size, &file_config[GTP_ADDR_LENGTH]);
	if (file_cfg_len < 0) {
...
static u8 gup_burn_proc(struct i2c_client *client, u8 *burn_buf, u16 start_addr, u16 total_length)
{
	...
	while (burn_length < total_length) {
		...
		for (retry = 0; retry < MAX_FRAME_CHECK_TIME; retry++) {
			...
			if (memcmp(&wr_buf[GTP_ADDR_LENGTH], &rd_buf[GTP_ADDR_LENGTH],
				   frame_length)) {
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	...
	if (SEEK_SET == set_or_end) {
		...
		/* seek end */
		memcpy(buf, &update_msg.fw_data[update_msg.fw_total_len + FW_HEAD_LENGTH - offset],
		       length);
	}
				    ...
				    u32 len, u8 bank_cmd)
{
	...
	if (FAIL == ret) {
		dev_err(&client->dev, "[burn_fw_app_section]recall check %dk firmware fail.",
			len / 1024);
		return FAIL;
	}
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	ret = gup_load_section_file(fw_gwake, start_index + 2 * FW_SECTION_LENGTH,
	...
	ret = gup_load_section_file(fw_gwake, start_index + 3 * FW_SECTION_LENGTH,
				    FW_SECTION_LENGTH, SEEK_SET);
	if (FAIL == ret) {
...
EXPORT_SYMBOL_GPL(gup_init_update_proc);
