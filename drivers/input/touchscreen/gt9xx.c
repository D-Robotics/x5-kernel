...
int gtp_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
	...
	if (likely(len < sizeof(get_buf))) {
		...
		msgs[1].buf = kzalloc(len > I2C_MAX_TRANSFER_SIZE ? I2C_MAX_TRANSFER_SIZE : len,
				      GFP_KERNEL);
		if (!msgs[1].buf)
			return -ENOMEM;
	}
...

{
	...
	if (likely(len < sizeof(put_buf))) {
		...
		msg.buf = kmalloc(len > I2C_MAX_TRANSFER_SIZE ? I2C_MAX_TRANSFER_SIZE : len,
				  GFP_KERNEL);
		if (!msg.buf)
			return -ENOMEM;
	}
...
EXPORT_SYMBOL_GPL(gtp_i2c_write);
	...
	if (key_value & 0xf0 || pre_key & 0xf0) {
		...
		for (i = 0; i < ts->pdata->key_nums; i++) {
			if ((pre_key | key_value) & (0x01 << i))
				input_report_key(ts->input_dev, ts->pdata->key_map[i],
						 key_value & (0x01 << i));
		}
		input_sync(ts->input_dev);
		pre_key = key_value;
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	dev_dbg(&ts->client->dev, "Config Groups\' Lengths: %d, %d, %d, %d, %d, %d",
		cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_info_len[3], cfg_info_len[4],
		cfg_info_len[5]);
#endif
	...
	ret = gtp_parse_dt_cfg(&ts->client->dev, &cfg->data[GTP_ADDR_LENGTH], &cfg->length,
			       sensor_id);
	if (ret < 0) {
...
static ssize_t gtp_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	if (ret < 0) {
		data_len +=
			snprintf(ptr + data_len, 4096 - data_len, "Failed read real config data\n");
	} else {
		for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
...
EXPORT_SYMBOL(gtp_ascii_to_array);
...
static struct attribute *gtp_attrs[] = {&dev_attr_workmode.attr,   &dev_attr_productinfo.attr,
					&dev_attr_dofwupdate.attr, &dev_attr_drv_irq.attr,
...
static int gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(ts->pdata->irq_gpio) || ts->client->irq > 0) {
		...
		ret = request_threaded_irq(ts->client->irq, NULL, gtp_irq_handler,
					   ts->pdata->irq_flags | IRQF_ONESHOT, ts->client->name,
					   ts);
		if (ret < 0) {
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->type_a_report) {
		input_mt_init_slots(
			ts->input_dev, 16,
			INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED); // in case of "out of memory"
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0, ts->pdata->max_touch_width, 0,
			     0);
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, ts->pdata->max_touch_pressure, 0,
			     0);
...
static void gtp_parse_dt_coords(struct device *dev, struct goodix_ts_platform_data *pdata)
{
	...
	dev_info(dev, "touch input parameters is [id x y w p]<%d %d %d %d %d>\n",
		 pdata->max_touch_id, pdata->abs_size_x, pdata->abs_size_y, pdata->max_touch_width,
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	// struct property *prop;
	char hard_code_cfg[] = {
		0x41, 0xD0, 0x02, 0x00, 0x05, 0x05, 0x35, 0x20, 0x22, 0x08, 0x28, 0x05, 0x5A, 0x3C,
		0x03, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x1A, 0x1E, 0x14, 0x87,
		0x29, 0x09, 0xCD, 0xCF, 0xB5, 0x06, 0x00, 0x00, 0x00, 0x20, 0x02, 0x10, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB4, 0xEF, 0x94, 0xD5, 0x02,
		0x08, 0x00, 0x00, 0x04, 0x87, 0xB9, 0x00, 0x82, 0xC4, 0x00, 0x7E, 0xCF, 0x00, 0x7B,
		0xDB, 0x00, 0x78, 0xE8, 0x00, 0x78, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		...
		0x06, 0x08, 0x0A, 0x0C, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0x54, 0x01};
	memcpy(cfg, hard_code_cfg, 186);
    *cfg_len = 186;
	return 0;
}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		...
		dev_dbg(&ts->client->dev, "[Esd]0x8040 = 0x%02X, 0x8041 = 0x%02X", esd_buf[2],
			esd_buf[3]);
...
static const struct i2c_device_id gtp_device_id[] = {{GTP_I2C_NAME, 0}, {}};
