...
static void hantro_job_finish_no_pm(struct hantro_dev *vpu, struct hantro_ctx *ctx,
				    enum vb2_buffer_state result)
{
	...
	v4l2_m2m_buf_done_and_job_finish(ctx->dev->m2m_dev, ctx->fh.m2m_ctx, result);
}

static void hantro_job_finish(struct hantro_dev *vpu, struct hantro_ctx *ctx,
			      enum vb2_buffer_state result)
{
...
void hantro_watchdog(struct work_struct *work)
{
	...
	vpu = container_of(to_delayed_work(work), struct hantro_dev, watchdog_work);
...
void hantro_end_prepare_run(struct hantro_ctx *ctx)
{
	...
	src_buf = hantro_get_src_buf(ctx);
	v4l2_ctrl_request_complete(src_buf->vb2_buf.req_obj.req, &ctx->ctrl_handler);

	/* Kick the watchdog. */
	schedule_delayed_work(&ctx->dev->watchdog_work, msecs_to_jiffies(2000));
}
...
#define HANTRO_JPEG_ACTIVE_MARKERS                                    \
	(V4L2_JPEG_ACTIVE_MARKER_APP0 | V4L2_JPEG_ACTIVE_MARKER_COM | \
...
static int hantro_ctrls_setup(struct hantro_dev *vpu, struct hantro_ctx *ctx, int allowed_codecs)
{
...
{
	struct hantro_ctx *ctx = container_of(filp->private_data, struct hantro_ctx, fh);
...
static const struct of_device_id of_hantro_match[] = {
#ifdef CONFIG_VIDEO_HANTRO_ROCKCHIP
	{
		...
		.data	    = &px30_vpu_variant,
	},
	{
		.compatible = "rockchip,rk3036-vpu",
		.data	    = &rk3036_vpu_variant,
	},
	{
		.compatible = "rockchip,rk3066-vpu",
		.data	    = &rk3066_vpu_variant,
	},
	{
		.compatible = "rockchip,rk3288-vpu",
		.data	    = &rk3288_vpu_variant,
	},
	{
		.compatible = "rockchip,rk3328-vpu",
		.data	    = &rk3328_vpu_variant,
	},
	{
		.compatible = "rockchip,rk3399-vpu",
		.data	    = &rk3399_vpu_variant,
	},
	{
		.compatible = "rockchip,rk3568-vepu",
		.data	    = &rk3568_vepu_variant,
	},
	{
		.compatible = "rockchip,rk3568-vpu",
		.data	    = &rk3568_vpu_variant,
	},
#endif
#ifdef CONFIG_VIDEO_HANTRO_IMX8M
	{
		.compatible = "nxp,imx8mm-vpu-g1",
		.data	    = &imx8mm_vpu_g1_variant,
	},
	{
		.compatible = "nxp,imx8mq-vpu",
		.data	    = &imx8mq_vpu_variant,
	},
...
#ifdef CONFIG_VIDEO_HANTRO_SAMA5D4
	{
		...
		.data	    = &sama5d4_vdec_variant,
	},
#endif
#ifdef CONFIG_VIDEO_HANTRO_SUNXI
	{
		.compatible = "allwinner,sun50i-h6-vpu-g2",
		.data	    = &sunxi_vpu_variant,
	},
#endif
	{/* sentinel */}};
MODULE_DEVICE_TABLE(of, of_hantro_match);

static int hantro_register_entity(struct media_device *mdev, struct media_entity *entity,
				  const char *entity_name, struct media_pad *pads, int num_pads,
				  int function, struct video_device *vdev)
{
	...
	name = devm_kasprintf(mdev->dev, GFP_KERNEL, "%s-%s", vdev->name, entity_name);
	if (!name)
		return -ENOMEM;
...
static int hantro_attach_func(struct hantro_dev *vpu, struct hantro_func *func)
{
	...
	ret = hantro_register_entity(mdev, &func->vdev.entity, "source", &func->source_pad, 1,
	...
	ret = hantro_register_entity(mdev, &func->proc, "proc", func->proc_pads, 2, func->id,
				     &func->vdev);
	if (ret)
		goto err_rel_entity0;

	func->sink_pad.flags = MEDIA_PAD_FL_SINK;
	ret		     = hantro_register_entity(mdev, &func->sink, "sink", &func->sink_pad, 1,
						      MEDIA_ENT_F_IO_V4L, &func->vdev);
	if (ret)
	...
	func->intf_devnode = media_devnode_create(mdev, MEDIA_INTF_T_V4L_VIDEO, 0, VIDEO_MAJOR,
						  func->vdev.minor);
	...
	link = media_create_intf_link(&func->vdev.entity, &func->intf_devnode->intf,
				      MEDIA_LNK_FL_IMMUTABLE | MEDIA_LNK_FL_ENABLED);
	if (!link) {
		ret = -ENOMEM;
		goto err_rm_devnode;
...
static void hantro_remove_func(struct hantro_dev *vpu, unsigned int funcid)
{
	struct hantro_func *func;
...
static int hantro_probe(struct platform_device *pdev)
{
	...
	vpu->clocks = devm_kcalloc(&pdev->dev, vpu->variant->num_clocks, sizeof(*vpu->clocks),
				   GFP_KERNEL);
	if (!vpu->clocks)
		return -ENOMEM;
	...
	if (vpu->variant->num_clocks > 1) {
		...
		ret = devm_clk_bulk_get(&pdev->dev, vpu->variant->num_clocks, vpu->clocks);
		if (ret)
			return ret;
	} else {
		/*
	...
	vpu->reg_bases = devm_kcalloc(&pdev->dev, num_bases, sizeof(*vpu->reg_bases), GFP_KERNEL);
	if (!vpu->reg_bases)
		return -ENOMEM;
	...
	for (i = 0; i < vpu->variant->num_irqs; i++) {
		...
		ret = devm_request_irq(vpu->dev, irq, vpu->variant->irqs[i].handler, 0,
	...
	strscpy(vpu->mdev.bus_info, "platform: " DRIVER_NAME, sizeof(vpu->mdev.bus_info));
