...
static void prepare_tile_info_buffer(struct hantro_ctx *ctx)
{
	...
	pic_width_in_ctbs = (sps->pic_width_in_luma_samples + (1 << max_log2_ctb_size) - 1) >>
			    max_log2_ctb_size;
		...
		if (!uniform_spacing) {
			...
			for (i = 0, prev_h = 0; i < num_tile_rows; i++) {
				...
				for (j = 0, prev_w = 0; j < num_tile_cols; j++) {
					...
					*p++ = tmp - prev_w;
					*p++ = h;
					if (j == 0 && (pps->column_width_minus1[0] + 1) == 1 &&
					    ctb_size == 16)
						no_chroma = 1;
					prev_w = tmp;
				}
...
static void set_params(struct hantro_ctx *ctx)
{
	...
	hantro_reg_write(vpu, &g2_pic_width_4x4, (pic_width_in_min_cbs * min_cb_size) / 4);
	hantro_reg_write(vpu, &g2_pic_height_4x4, (pic_height_in_min_cbs * min_cb_size) / 4);

	hantro_reg_write(vpu, &hevc_max_inter_hierdepth, sps->max_transform_hierarchy_depth_inter);
	hantro_reg_write(vpu, &hevc_max_intra_hierdepth, sps->max_transform_hierarchy_depth_intra);
	...
	hantro_reg_write(vpu, &g2_asym_pred_e, !!(sps->flags & V4L2_HEVC_SPS_FLAG_AMP_ENABLED));
	...
	hantro_reg_write(vpu, &hevc_parallel_merge, pps->log2_parallel_merge_level_minus2 + 2);
	hantro_reg_write(vpu, &g2_pcm_filt_d,
			 !!(sps->flags & V4L2_HEVC_SPS_FLAG_PCM_LOOP_FILTER_DISABLED));
	hantro_reg_write(vpu, &g2_pcm_e, !!(sps->flags & V4L2_HEVC_SPS_FLAG_PCM_ENABLED));
	...
	hantro_reg_write(vpu, &g2_weight_pred_e, !!(pps->flags & V4L2_HEVC_PPS_FLAG_WEIGHTED_PRED));
	hantro_reg_write(vpu, &g2_cabac_init_present,
			 !!(pps->flags & V4L2_HEVC_PPS_FLAG_CABAC_INIT_PRESENT));
	...
	hantro_reg_write(vpu, &g2_refidx0_active, pps->num_ref_idx_l0_default_active_minus1 + 1);
	hantro_reg_write(vpu, &g2_refidx1_active, pps->num_ref_idx_l1_default_active_minus1 + 1);
...
static void set_ref_pic_list(struct hantro_ctx *ctx)
{
	...
	static const struct hantro_reg ref_pic_regs0[]		 = {
		  hevc_rlist_f0,  hevc_rlist_f1,  hevc_rlist_f2,  hevc_rlist_f3,
		  hevc_rlist_f4,  hevc_rlist_f5,  hevc_rlist_f6,  hevc_rlist_f7,
		  hevc_rlist_f8,  hevc_rlist_f9,  hevc_rlist_f10, hevc_rlist_f11,
		  hevc_rlist_f12, hevc_rlist_f13, hevc_rlist_f14, hevc_rlist_f15,
	  };
	static const struct hantro_reg ref_pic_regs1[] = {
		hevc_rlist_b0,	hevc_rlist_b1,	hevc_rlist_b2,	hevc_rlist_b3,
		hevc_rlist_b4,	hevc_rlist_b5,	hevc_rlist_b6,	hevc_rlist_b7,
		hevc_rlist_b8,	hevc_rlist_b9,	hevc_rlist_b10, hevc_rlist_b11,
		hevc_rlist_b12, hevc_rlist_b13, hevc_rlist_b14, hevc_rlist_b15,
	};
	unsigned int i, j;
...
static int set_ref(struct hantro_ctx *ctx)
{
	...
	static const struct hantro_reg cur_poc[] = {
		hevc_cur_poc_00, hevc_cur_poc_01, hevc_cur_poc_02, hevc_cur_poc_03,
		hevc_cur_poc_04, hevc_cur_poc_05, hevc_cur_poc_06, hevc_cur_poc_07,
		hevc_cur_poc_08, hevc_cur_poc_09, hevc_cur_poc_10, hevc_cur_poc_11,
		hevc_cur_poc_12, hevc_cur_poc_13, hevc_cur_poc_14, hevc_cur_poc_15,
	};
	unsigned int i;

	max_ref_frames = decode_params->num_poc_lt_curr + decode_params->num_poc_st_curr_before +
	...
	dpb_longterm_e = 0;
	for (i = 0;
	     i < decode_params->num_active_dpb_entries && i < (V4L2_HEVC_DPB_ENTRIES_NUM_MAX - 1);
	     i++) {
