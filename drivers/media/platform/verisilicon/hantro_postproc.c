...
#define HANTRO_PP_REG_WRITE(vpu, reg_name, val)                                \
	{                                                                      \
		hantro_reg_write(vpu, &hantro_g1_postproc_regs.reg_name, val); \
	}

#define HANTRO_PP_REG_WRITE_S(vpu, reg_name, val)                                \
	{                                                                        \
		hantro_reg_write_s(vpu, &hantro_g1_postproc_regs.reg_name, val); \
	}
...
static void hantro_postproc_g2_enable(struct hantro_ctx *ctx)
{
	...
	dst_dma	      = vb2_dma_contig_plane_dma_addr(&dst_buf->vb2_buf, 0);
	chroma_offset = ctx->dst_fmt.plane_fmt[0].bytesperline * ctx->dst_fmt.height;
...
int hantro_postproc_alloc(struct hantro_ctx *ctx)
{
	...
	const struct vb2_queue *cap_queue = &m2m_ctx->cap_q_ctx.q;
	...
	if (!fmt)
		return -EINVAL;
	v4l2_fill_pixfmt_mp(&pix_mp, fmt->fourcc, ctx->src_fmt.width, ctx->src_fmt.height);

	buf_size = pix_mp.plane_fmt[0].sizeimage;
	if (ctx->vpu_src_fmt->fourcc == V4L2_PIX_FMT_H264_SLICE)
		buf_size += hantro_h264_mv_size(pix_mp.width, pix_mp.height);
	else if (ctx->vpu_src_fmt->fourcc == V4L2_PIX_FMT_VP9_FRAME)
		buf_size += hantro_vp9_mv_size(pix_mp.width, pix_mp.height);
	else if (ctx->vpu_src_fmt->fourcc == V4L2_PIX_FMT_HEVC_SLICE)
		buf_size += hantro_hevc_mv_size(pix_mp.width, pix_mp.height);
	...
	for (i = 0; i < num_buffers; ++i) {
		...
		priv->cpu =
			dma_alloc_attrs(vpu->dev, buf_size, &priv->dma, GFP_KERNEL, priv->attrs);
		if (!priv->cpu)
			return -ENOMEM;
		priv->size = buf_size;
	}
