...
static void rockchip_vpu2_mpeg2_dec_set_quantisation(struct hantro_dev *vpu, struct hantro_ctx *ctx)
{
...
static void rockchip_vpu2_mpeg2_dec_set_buffers(struct hantro_dev *vpu, struct hantro_ctx *ctx,
...
int rockchip_vpu2_mpeg2_dec_run(struct hantro_ctx *ctx)
{
	...
	reg = VDPU_REG_DEC_ADV_PRE_DIS(0) | VDPU_REG_DEC_SCMD_DIS(0) | VDPU_REG_FILTERING_DIS(1) |
	...
	reg = VDPU_REG_APF_THRESHOLD(8) | VDPU_REG_STARTMB_X(0) | VDPU_REG_STARTMB_Y(0);
	vdpu_write_relaxed(vpu, reg, VDPU_SWREG(52));

	reg = VDPU_REG_DEC_MODE(5);
	vdpu_write_relaxed(vpu, reg, VDPU_SWREG(53));

	reg = VDPU_REG_DEC_STRENDIAN_E(1) | VDPU_REG_DEC_STRSWAP32_E(1) |
	      VDPU_REG_DEC_OUTSWAP32_E(1) | VDPU_REG_DEC_INSWAP32_E(1) |
	      VDPU_REG_DEC_OUT_ENDIAN(1) | VDPU_REG_DEC_IN_ENDIAN(1);
	vdpu_write_relaxed(vpu, reg, VDPU_SWREG(54));

	reg = VDPU_REG_DEC_DATA_DISC_E(0) | VDPU_REG_DEC_MAX_BURST(16) | VDPU_REG_DEC_AXI_WR_ID(0) |
	...
	reg = VDPU_REG_RLC_MODE_E(0) |
	      ...
	      VDPU_REG_FWD_INTERLACE_E(0) | VDPU_REG_WRITE_MVS_E(0) | VDPU_REG_DEC_TIMEOUT_E(1) |
	...
	reg = VDPU_REG_ALT_SCAN_FLAG_E(pic->flags & V4L2_MPEG2_PIC_FLAG_ALT_SCAN) |
	      ...
	      VDPU_REG_FCODE_BWD_VER(pic->f_code[1][1]) | VDPU_REG_MV_ACCURACY_FWD(1) |
	      VDPU_REG_MV_ACCURACY_BWD(1);
	vdpu_write_relaxed(vpu, reg, VDPU_SWREG(136));

	rockchip_vpu2_mpeg2_dec_set_quantisation(vpu, ctx);

	rockchip_vpu2_mpeg2_dec_set_buffers(vpu, ctx, &src_buf->vb2_buf, &dst_buf->vb2_buf, seq,
					    pic);
