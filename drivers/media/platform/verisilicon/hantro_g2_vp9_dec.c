...
static size_t mv_offset(const struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params)
{
...
static struct hantro_decoded_buffer *get_ref_buf(struct hantro_ctx *ctx,
						 struct vb2_v4l2_buffer *dst, u64 timestamp)
{
...
static void config_output(struct hantro_ctx *ctx, struct hantro_decoded_buffer *dst,
			  const struct v4l2_ctrl_vp9_frame *dec_params)
{
...
static void config_ref(struct hantro_ctx *ctx, struct hantro_decoded_buffer *dst,
		       const struct hantro_vp9_ref_reg *ref_reg,
		       const struct v4l2_ctrl_vp9_frame *dec_params, u64 ref_ts)
{
	struct hantro_decoded_buffer *buf;
				 ...
				 struct hantro_decoded_buffer *mv_ref)
{
	...
	mv_addr =
		hantro_get_dec_buf_addr(ctx, &mv_ref->base.vb.vb2_buf) + mv_offset(ctx, dec_params);
...
static void recompute_tile_rc_info(struct hantro_ctx *ctx, unsigned int tile_r, unsigned int tile_c,
...
static void fill_tile_info(struct hantro_ctx *ctx, unsigned int tile_r, unsigned int tile_c,
			   unsigned int sbs_r, unsigned int sbs_c, unsigned short *tile_mem)
{
	struct hantro_vp9_dec_hw_ctx *vp9_ctx = &ctx->vp9_dec;
	unsigned int i, j;
	bool first = true;
...
static void config_tiles(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params,
			 struct hantro_decoded_buffer *dst)
{
	struct hantro_vp9_dec_hw_ctx *vp9_ctx  = &ctx->vp9_dec;
	struct hantro_aux_buf *misc	       = &vp9_ctx->misc;
	const struct hantro_aux_buf *tile_edge = &vp9_ctx->tile_edge;
	dma_addr_t addr;
	...
	tile_mem = (unsigned short *)misc->cpu + vp9_ctx->tile_info_offset;
...
static void update_feat_and_flag(struct hantro_vp9_dec_hw_ctx *vp9_ctx,
				 const struct v4l2_vp9_segmentation *seg, unsigned int feature,
				 unsigned int segid)
{
...
static void config_segment(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params)
{
	...
	for (segid = 0; segid < 8; ++segid) {
		...
		if (segment_enabled) {
			if (update_data)
				update_feat_and_flag(vp9_ctx, seg, feat_id, segid);
			feat_val = v4l2_vp9_seg_feat_enabled(vp9_ctx->feature_enabled, feat_id,
							     segid) ?
					   1 :
					   0;
		}
...
static void config_bit_depth(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params)
{
	if (ctx->dev->variant->legacy_regs) {
...
{
	return quant->base_q_idx == 0 && quant->delta_q_uv_ac == 0 && quant->delta_q_uv_dc == 0 &&
	       quant->delta_q_y_dc == 0;
}

static void config_quant(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params)
{
...
static void config_others(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params,
			  bool intra_only, bool resolution_change)
{
	...
	hantro_reg_write(
		ctx->dev, &vp9_mcomp_filt_type,
		intra_only ? 0 : hantro_interp_filter_from_v4l2(dec_params->interpolation_filter));
				      ...
				      const struct v4l2_ctrl_vp9_frame *dec_params)
{
	...
	if (comp_ref_allowed) {
		if (last_ref_frame_sign_bias == golden_ref_frame_sign_bias) {
			comp_fixed_ref	= ALTREF_FRAME;
			comp_var_ref[0] = LAST_FRAME;
			comp_var_ref[1] = GOLDEN_FRAME;
		} else if (last_ref_frame_sign_bias == alt_ref_frame_sign_bias) {
...
#define INNER_LOOP                                                                        \
	do {                                                                              \
		for (m = 0; m < ARRAY_SIZE(adaptive->coef[0][0][0][0]); ++m) {            \
			memcpy(adaptive->coef[i][j][k][l][m], probs->coef[i][j][k][l][m], \
...
static void config_probs(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params)
{
	...
	for (i = 0; i < ARRAY_SIZE(all_probs->kf_y_mode_prob); ++i)
		for (j = 0; j < ARRAY_SIZE(all_probs->kf_y_mode_prob[0]); ++j) {
			memcpy(all_probs->kf_y_mode_prob[i][j], v4l2_vp9_kf_y_mode_prob[i][j],
			       ARRAY_SIZE(all_probs->kf_y_mode_prob[i][j]));

			all_probs->kf_y_mode_prob_tail[i][j][0] = v4l2_vp9_kf_y_mode_prob[i][j][8];
		}
	...
	for (i = 0; i < ARRAY_SIZE(adaptive->uv_mode); ++i) {
		memcpy(adaptive->uv_mode[i], probs->uv_mode[i], sizeof(adaptive->uv_mode[i]));
	...
	for (i = 0; i < ARRAY_SIZE(adaptive->y_mode); ++i) {
		memcpy(adaptive->y_mode[i], probs->y_mode[i], ARRAY_SIZE(adaptive->y_mode[i]));
	...
	for (i = 0; i < ARRAY_SIZE(adaptive->partition[1]); ++i) {
		memcpy(adaptive->partition[1][i], probs->partition[i], sizeof(probs->partition[i]));

		adaptive->partition[1][i][3] = 0;
	}

	memcpy(adaptive->interp_filter, probs->interp_filter, sizeof(adaptive->interp_filter));
...
static void config_counts(struct hantro_ctx *ctx)
{
	...
	const struct hantro_aux_buf *misc     = &vp9_dec->misc;
	dma_addr_t addr			      = misc->dma + vp9_dec->ctx_counters_offset;

	hantro_write_addr(ctx->dev, G2_VP9_CTX_COUNT_ADDR, addr);
}

static void config_seg_map(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params,
			   bool intra_only, bool update_map)
{
	...
	if (intra_only || (dec_params->flags & V4L2_VP9_FRAME_FLAG_ERROR_RESILIENT)) {
...
static void config_source(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params,
			  struct vb2_v4l2_buffer *vb2_src)
{
	...
	headres_size = dec_params->uncompressed_header_size + dec_params->compressed_header_size;
...
static void config_registers(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params,
			     struct vb2_v4l2_buffer *vb2_src, struct vb2_v4l2_buffer *vb2_dst)
{
	...
	resolution_change =
		dst->vp9.width != last->vp9.width || dst->vp9.height != last->vp9.height;
...
void hantro_g2_vp9_dec_done(struct hantro_ctx *ctx)
{
	...
	if (!(vp9_ctx->cur.flags & V4L2_VP9_FRAME_FLAG_PARALLEL_DEC_MODE)) {
		...
		bool frame_is_intra = vp9_ctx->cur.flags & (V4L2_VP9_FRAME_FLAG_KEY_FRAME |
		...
		*probs = vp9_ctx->frame_context[fctx_idx];
		...
		counts = &vp9_ctx->cnts;
		hantro_cnts =
			(struct symbol_counts *)vp9_ctx->misc.cpu + vp9_ctx->ctx_counters_offset;
		for (i = 0; i < ARRAY_SIZE(tx16p); ++i) {
			memcpy(tx16p[i], hantro_cnts->tx16x16_count[i],
		...
		v4l2_vp9_adapt_coef_probs(
			probs, counts,
			!vp9_ctx->last.valid || vp9_ctx->last.flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME,
			frame_is_intra);
