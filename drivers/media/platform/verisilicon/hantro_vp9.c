...
#define INNER_LOOP                                                                                 \
	do {                                                                                       \
		for (m = 0; m < ARRAY_SIZE(vp9_ctx->cnts.coeff[i][0][0][0]); ++m) {                \
			vp9_ctx->cnts.coeff[i][j][k][l][m]  = get_coeffs_arr(cnts, i, j, k, l, m); \
			vp9_ctx->cnts.eob[i][j][k][l][m][0] = &cnts->count_eobs[i][j][k][l][m];    \
			vp9_ctx->cnts.eob[i][j][k][l][m][1] = get_eobs1(cnts, i, j, k, l, m);      \
		}                                                                                  \
	} while (0)
