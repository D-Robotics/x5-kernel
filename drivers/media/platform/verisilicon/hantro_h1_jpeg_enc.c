...
static void hantro_h1_set_src_img_ctrl(struct hantro_dev *vpu, struct hantro_ctx *ctx)
{
	...
	reg = H1_REG_IN_IMG_CTRL_ROW_LEN(ctx->src_fmt.width) |
	      H1_REG_IN_IMG_CTRL_OVRFLR_D4(overfill_r / 4) | H1_REG_IN_IMG_CTRL_OVRFLB(overfill_b) |
	      H1_REG_IN_IMG_CTRL_FMT(ctx->vpu_src_fmt->enc_fmt);
	vepu_write_relaxed(vpu, reg, H1_REG_IN_IMG_CTRL);
}

static void hantro_h1_jpeg_enc_set_buffers(struct hantro_dev *vpu, struct hantro_ctx *ctx,
...
static void hantro_h1_jpeg_enc_set_qtable(struct hantro_dev *vpu, unsigned char *luma_qtable,
...
int hantro_h1_jpeg_enc_run(struct hantro_ctx *ctx)
{
	...
	vepu_write_relaxed(vpu, H1_REG_ENC_CTRL_ENC_MODE_JPEG, H1_REG_ENC_CTRL);

	hantro_h1_set_src_img_ctrl(vpu, ctx);
	hantro_h1_jpeg_enc_set_buffers(vpu, ctx, &src_buf->vb2_buf, &dst_buf->vb2_buf);
	hantro_h1_jpeg_enc_set_qtable(vpu, jpeg_ctx.hw_luma_qtable, jpeg_ctx.hw_chroma_qtable);

	reg = H1_REG_AXI_CTRL_OUTPUT_SWAP16 | H1_REG_AXI_CTRL_INPUT_SWAP16 |
	      H1_REG_AXI_CTRL_BURST_LEN(16) | H1_REG_AXI_CTRL_OUTPUT_SWAP32 |
	      H1_REG_AXI_CTRL_INPUT_SWAP32 | H1_REG_AXI_CTRL_OUTPUT_SWAP8 |
	...
	reg = H1_REG_ENC_CTRL_WIDTH(MB_WIDTH(ctx->src_fmt.width)) |
	      ...
	      H1_REG_ENC_CTRL_ENC_MODE_JPEG | H1_REG_ENC_PIC_INTRA | H1_REG_ENC_CTRL_EN_BIT;
