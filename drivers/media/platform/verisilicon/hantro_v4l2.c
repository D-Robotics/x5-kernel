...
static int hantro_set_fmt_out(struct hantro_ctx *ctx, struct v4l2_pix_format_mplane *pix_mp);
static int hantro_set_fmt_cap(struct hantro_ctx *ctx, struct v4l2_pix_format_mplane *pix_mp);
...
static const struct hantro_fmt *hantro_get_postproc_formats(const struct hantro_ctx *ctx,
							    unsigned int *num_fmts)
{
...
static bool hantro_check_depth_match(const struct hantro_ctx *ctx, const struct hantro_fmt *fmt)
{
...
static const struct hantro_fmt *hantro_find_format(const struct hantro_ctx *ctx, u32 fourcc)
{
	const struct hantro_fmt *formats;
...
const struct hantro_fmt *hantro_get_default_fmt(const struct hantro_ctx *ctx, bool bitstream)
{
	const struct hantro_fmt *formats;
	unsigned int i, num_fmts;
	...
	formats = hantro_get_formats(ctx, &num_fmts);
	for (i = 0; i < num_fmts; i++) {
		if (bitstream == (formats[i].codec_mode != HANTRO_MODE_NONE) &&
...
static int vidioc_querycap(struct file *file, void *priv, struct v4l2_capability *cap)
{
	...
	snprintf(cap->bus_info, sizeof(cap->bus_info), "platform: %s", vpu->dev->driver->name);
	return 0;
}
...
static int vidioc_enum_fmt(struct file *file, void *priv, struct v4l2_fmtdesc *f, bool capture)

{
	const struct hantro_ctx *ctx = fh_to_ctx(priv);
	 ...
	 * as a hint for applications on what's the preferred format.
	 */
	if (!capture)
		return -EINVAL;
	formats = hantro_get_postproc_formats(ctx, &num_fmts);
...
static int vidioc_g_fmt_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
	struct v4l2_pix_format_mplane *pix_mp = &f->fmt.pix_mp;
	const struct hantro_ctx *ctx	      = fh_to_ctx(priv);
...
static int hantro_try_fmt(const struct hantro_ctx *ctx, struct v4l2_pix_format_mplane *pix_mp,
			  enum v4l2_buf_type type)
{
	...
	vpu_debug(4, "trying format %c%c%c%c\n", (pix_mp->pixelformat & 0x7f),
		  (pix_mp->pixelformat >> 8) & 0x7f, (pix_mp->pixelformat >> 16) & 0x7f,
	...
	v4l2_apply_frmsize_constraints(&pix_mp->width, &pix_mp->height, &vpu_fmt->frmsize);
	...
	if (!coded) {
		...
		if (ctx->vpu_src_fmt->fourcc == V4L2_PIX_FMT_H264_SLICE &&
			...
			pix_mp->plane_fmt[0].sizeimage +=
			 ...
			 !hantro_needs_postproc(ctx, fmt))
			pix_mp->plane_fmt[0].sizeimage +=
				hantro_vp9_mv_size(pix_mp->width, pix_mp->height);
		else if (ctx->vpu_src_fmt->fourcc == V4L2_PIX_FMT_HEVC_SLICE &&
			 !hantro_needs_postproc(ctx, fmt))
			pix_mp->plane_fmt[0].sizeimage +=
				hantro_hevc_mv_size(pix_mp->width, pix_mp->height);
		 ...
		 */
		pix_mp->plane_fmt[0].sizeimage =
			fmt->header_size + pix_mp->width * pix_mp->height * fmt->max_depth;
	}

	return 0;
}

static int vidioc_try_fmt_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
	return hantro_try_fmt(fh_to_ctx(priv), &f->fmt.pix_mp, f->type);
}

static int vidioc_try_fmt_out_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
	return hantro_try_fmt(fh_to_ctx(priv), &f->fmt.pix_mp, f->type);
}

static void hantro_reset_fmt(struct v4l2_pix_format_mplane *fmt, const struct hantro_fmt *vpu_fmt)
{
...
static void hantro_reset_raw_fmt(struct hantro_ctx *ctx)
{
...
static void hantro_update_requires_hold_capture_buf(struct hantro_ctx *ctx, u32 fourcc)
{
	struct vb2_queue *vq;

	vq = v4l2_m2m_get_vq(ctx->fh.m2m_ctx, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
...
static int hantro_set_fmt_out(struct hantro_ctx *ctx, struct v4l2_pix_format_mplane *pix_mp)
{
	...
	vq = v4l2_m2m_get_vq(ctx->fh.m2m_ctx, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
	hantro_try_fmt(ctx, pix_mp, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
	// if (ret)
	//	return ret;
	...
	if (!ctx->is_encoder) {
		...
		if (vb2_is_streaming(vq) ||
		    (vb2_is_busy(vq) && pix_mp->pixelformat != ctx->src_fmt.pixelformat))
			return -EBUSY;
		/*
		 ...
		 */
		peer_vq = v4l2_m2m_get_vq(ctx->fh.m2m_ctx, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
...
static int hantro_set_fmt_cap(struct hantro_ctx *ctx, struct v4l2_pix_format_mplane *pix_mp)
{
	...
	if (ctx->is_encoder) {
		 ...
		 */
		peer_vq = v4l2_m2m_get_vq(ctx->fh.m2m_ctx, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
		if (vb2_is_busy(peer_vq) &&
		    (pix_mp->pixelformat != ctx->dst_fmt.pixelformat ||
		     pix_mp->height != ctx->dst_fmt.height || pix_mp->width != ctx->dst_fmt.width))
			return -EBUSY;
	}

	hantro_try_fmt(ctx, pix_mp, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
	// if (ret)
	//	return ret;
...
static int vidioc_s_fmt_out_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
	return hantro_set_fmt_out(fh_to_ctx(priv), &f->fmt.pix_mp);
}

static int vidioc_s_fmt_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
	return hantro_set_fmt_cap(fh_to_ctx(priv), &f->fmt.pix_mp);
}

static int vidioc_g_selection(struct file *file, void *priv, struct v4l2_selection *sel)
{
	struct hantro_ctx *ctx = fh_to_ctx(priv);

	/* Crop only supported on source. */
	if (!ctx->is_encoder || sel->type != V4L2_BUF_TYPE_VIDEO_OUTPUT)
		return -EINVAL;
...
static int vidioc_s_selection(struct file *file, void *priv, struct v4l2_selection *sel)
{
	struct hantro_ctx *ctx = fh_to_ctx(priv);
...
static const struct v4l2_event hantro_eos_event = {.type = V4L2_EVENT_EOS};

static int vidioc_encoder_cmd(struct file *file, void *priv, struct v4l2_encoder_cmd *ec)
{
	struct hantro_ctx *ctx = fh_to_ctx(priv);
	int ret;
	...
	if (ec->cmd == V4L2_ENC_CMD_STOP && v4l2_m2m_has_stopped(ctx->fh.m2m_ctx))
...
static int hantro_buf_plane_check(struct vb2_buffer *vb, struct v4l2_pix_format_mplane *pixfmt)
{
	unsigned int sz;
	int i;
...
{
	const struct hantro_ctx *ctx = vb2_get_drv_priv(q);
...
static void hantro_stop_streaming(struct vb2_queue *q)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(q->type) && v4l2_m2m_has_stopped(ctx->fh.m2m_ctx))
		v4l2_event_queue_fh(&ctx->fh, &hantro_eos_event);
}
