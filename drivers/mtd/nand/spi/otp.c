 ...
 *
 * Use the main data area size (not page+OOB). GD5F1GQ5 documents an 8KB OTP
 * region as 4 data pages; MTD_OPS_RAW cache ops only DMA the data area when
 * ooblen is zero, so sizing by page+OOB would return stale OOB bytes on read.
 *
...
/**
 * spinand_fact_otp_read() - Read from factory OTP area
 * @spinand: the spinand device
...
/**
 * spinand_user_otp_read() - Read from user OTP area
 * @spinand: the spinand device
 * @ofs: the offset to read
 * @len: the number of data bytes to read
 * @retlen: the pointer to variable to store the number of read bytes
...
/**
 * spinand_user_otp_write() - Write to user OTP area
 * @spinand: the spinand device
