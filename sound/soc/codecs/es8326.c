...
static int es8326_calibrate(struct snd_soc_component *component)
{
	...
	if ((es8326->version == ES8326_VERSION_B) && (es8326->calibrated == false)) {
		...
		regmap_write(es8326->regmap, ES8326_CLK_INV, 0xc0);//0x02
		regmap_write(es8326->regmap, ES8326_CLK_DIV1, 0x03);//0x04
		regmap_write(es8326->regmap, ES8326_CLK_DLL, 0x30);//0x06
		regmap_write(es8326->regmap, ES8326_CLK_MUX, 0xed);//0x07
		regmap_write(es8326->regmap, ES8326_CLK_DAC_SEL, 0x08);//0x09
		regmap_write(es8326->regmap, ES8326_CLK_TRI, 0xc1);//0x0e
		regmap_write(es8326->regmap, ES8326_DAC_MUTE, 0x03);//0x14
		regmap_write(es8326->regmap, ES8326_ANA_VSEL, 0x7f);//0x1c
		regmap_write(es8326->regmap, ES8326_VMIDLOW, 0x23);//0x22
		regmap_write(es8326->regmap, ES8326_DAC2HPMIX, 0x88);//0x25
		usleep_range(15000, 20000);
		regmap_write(es8326->regmap, ES8326_HP_OFFSET_CAL, 0x8c);//0x4A
		usleep_range(15000, 20000);
		regmap_write(es8326->regmap, ES8326_RESET, 0xc0);//0x00
		usleep_range(15000, 20000);

		regmap_write(es8326->regmap, ES8326_HP_OFFSET_CAL, ES8326_HP_OFF);//0x4A
		regmap_read(es8326->regmap, ES8326_CSM_MUTE_STA, &reg);//0xFC
		if ((reg & 0xf0) != 0x40)
			msleep(50);

		regmap_write(es8326->regmap, ES8326_HP_CAL, 0xd4);//0x27
		msleep(200);
		regmap_write(es8326->regmap, ES8326_HP_CAL, 0x4d);//0x27
		msleep(200);
		regmap_write(es8326->regmap, ES8326_HP_CAL, ES8326_HP_OFF);//0x27
		regmap_read(es8326->regmap, ES8326_HPL_OFFSET_INI, &offset_l);//0x4B
		regmap_read(es8326->regmap, ES8326_HPR_OFFSET_INI, &offset_r);//0x4C
		regmap_write(es8326->regmap, ES8326_HP_OFFSET_CAL, 0x8c);    //0x4A
		regmap_write(es8326->regmap, ES8326_HPL_OFFSET_INI, offset_l);//0x4B
		regmap_write(es8326->regmap, ES8326_HPR_OFFSET_INI, offset_r);//0x4C
		regmap_write(es8326->regmap, ES8326_CLK_INV, 0x00);//0x02
...
static int es8326_resume(struct snd_soc_component *component)
{
	...
	regmap_write(es8326->regmap, ES8326_RESET, 0x1f);//0x00
	regmap_write(es8326->regmap, ES8326_VMIDSEL, 0x0E);//0x18  defalut 0x0E  00 :vmid power down 
	regmap_write(es8326->regmap, ES8326_ANA_LP, 0xf0);//0x19
	usleep_range(10000, 15000);
	regmap_write(es8326->regmap, ES8326_HPJACK_TIMER, 0xd9);//0x56
	regmap_write(es8326->regmap, ES8326_ANA_MICBIAS, 0xd8);//0x1b
	/* set headphone default type and detect pin */
	regmap_write(es8326->regmap, ES8326_HPDET_TYPE, 0x83);  //0x57
	regmap_write(es8326->regmap, ES8326_CLK_RESAMPLE, 0x05);//0x03

	/* set internal oscillator as clock source of headpone cp */
	regmap_write(es8326->regmap, ES8326_CLK_DIV_CPC, 0x89);//0x0c
	regmap_write(es8326->regmap, ES8326_CLK_CTL, ES8326_CLK_ON);//0x01
	/* clock manager reset release */
	regmap_write(es8326->regmap, ES8326_RESET, 0x17);//0x00
	/* set headphone detection as half scan mode */
	regmap_write(es8326->regmap, ES8326_HP_MISC, 0x3d);//0xF7
	regmap_write(es8326->regmap, ES8326_PULLUP_CTL, 0x00);//0xF9

	/* enable headphone driver */
	regmap_write(es8326->regmap, ES8326_HP_VOL, 0xc4);//0x26
	regmap_write(es8326->regmap, ES8326_HP_DRIVER, 0xa7);//0x24
	usleep_range(2000, 5000);
	regmap_write(es8326->regmap, ES8326_HP_DRIVER_REF, 0x23);//0x28
	regmap_write(es8326->regmap, ES8326_HP_DRIVER_REF, 0x33);//0x28
	regmap_write(es8326->regmap, ES8326_HP_DRIVER, 0xa1);//0x24

	regmap_write(es8326->regmap, ES8326_CLK_INV, 0x00);//0x02
	regmap_write(es8326->regmap, ES8326_CLK_VMIDS1, 0xc4);//0x10
	regmap_write(es8326->regmap, ES8326_CLK_VMIDS2, 0x81);//0x11
	regmap_write(es8326->regmap, ES8326_CLK_CAL_TIME, 0x00);//0x12
	/* calibrate for B version */
	es8326_calibrate(component);
	regmap_write(es8326->regmap, ES8326_DAC_CROSSTALK, 0xaa);//0x55
	regmap_write(es8326->regmap, ES8326_DAC_RAMPRATE, 0x00);//0x4E
	/* turn off headphone out */
	regmap_write(es8326->regmap, ES8326_HP_CAL, 0x00);//0x27
	/* set ADC and DAC in low power mode */
	regmap_write(es8326->regmap, ES8326_ANA_LP, 0xf0);//0x19

	regmap_write(es8326->regmap, ES8326_ANA_VSEL, 0x7F);//0x1c
	/* select vdda as micbias source */
	regmap_write(es8326->regmap, ES8326_VMIDLOW, 0x03);//0x22
	/* set dac dsmclip = 1 */
	regmap_write(es8326->regmap, ES8326_DAC_DSM, 0x08);//0x4D
	regmap_write(es8326->regmap, ES8326_DAC_VPPSCALE, 0x15);//0x4F

	regmap_write(es8326->regmap, ES8326_HPDET_TYPE, 0x80 |
			((es8326->version == ES8326_VERSION_B) ?
			(ES8326_HP_DET_SRC_PIN9 | es8326->jack_pol) :
			(ES8326_HP_DET_SRC_PIN9 | es8326->jack_pol | 0x04)));//0x57
	usleep_range(5000, 10000);
	es8326_enable_micbias(es8326->component);
	usleep_range(50000, 70000);
	regmap_update_bits(es8326->regmap, ES8326_HPDET_TYPE, 0x03, 0x00);
	regmap_write(es8326->regmap, ES8326_INT_SOURCE, ES8326_INT_SRC_PIN9);//0x58
	regmap_write(es8326->regmap, ES8326_INTOUT_IO,
		     es8326->interrupt_clk);//0x59
	regmap_write(es8326->regmap, ES8326_SDINOUT1_IO,
		    (ES8326_IO_DMIC_CLK << ES8326_SDINOUT1_SHIFT));//0x5A
	regmap_write(es8326->regmap, ES8326_SDINOUT23_IO, ES8326_IO_INPUT);//0x5B

	regmap_write(es8326->regmap, ES8326_ANA_PDN, 0x00);//0x16
	regmap_write(es8326->regmap, ES8326_RESET, ES8326_CSM_ON);//0x00
	...
	regmap_write(es8326->regmap, ES8326_ADC_MUTE, 0x0f);//0x15
...
static const struct dev_pm_ops es8326_i2c_pm_ops = {
	.suspend = es8326_i2c_suspend,
	.resume = es8326_i2c_resume,
};
